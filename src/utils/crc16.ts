/**
 * Computes CRC16‑XModem checksum of a payload.
 * Returns a 2‑byte Uint8Array in little‑endian order.
 */
export function crc16xmodem(payload: Uint8Array): Uint8Array {
  let crc = 0;
  for (let i = 0; i < payload.length; i++) {
    const byte = payload[i];
    const lookupIndex = ((crc >> 8) ^ byte) & 0xff;
    crc = ((crc << 8) ^ CRC16_XMODEM_TABLE[lookupIndex]) & 0xffff;
  }
...
const CRC16_XMODEM_TABLE: number[] = [
